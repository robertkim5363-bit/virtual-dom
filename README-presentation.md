...
**미션**  
위성사진 한 장이면 주변 지형을 자동으로 샘플링해서, 은폐 대상을 최소한의 DOM 조작만으로 감쪽같이 위장하는 렌더링 엔진을 만든다.
...
### 3. 테스트 케이스
우리는 엔진이 단순히 동작하는 수준이 아니라 실제로 검증되도록, 발표용으로 재현 가능한 7개의 테스트 케이스를 만들었다.

이 테스트 케이스에서 Diff가 정확한지, Patch가 최소 변경으로 반영되는지, Undo / Redo가 안정적으로 복원되는지 확인할 수 있습니다.
...
### 그래프 설명
- `5개`는 핵심 Diff 유형 검증이다.
- `1개`는 변경이 없을 때도 엔진이 안정적인지 보는 No-op 검증이다.
- `1개`는 상태 복원 기능인 Undo / Redo 검증이다.
